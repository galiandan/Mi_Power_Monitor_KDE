...
安装脚本会从仓库内的 Go 源码构建后端，将它放在 `~/.local/bin/xiaomi-power`，并通过 `kpackagetool6` 安装 Plasma 小组件到 `${XDG_DATA_HOME:-~/.local/share}/plasma/plasmoids/com.github.galiandan.mipowermonitor/`。首次安装优先使用米家二维码登录：脚本会尝试自动打开本机浏览器中的登录页面，并显示网址；若浏览器没有弹出，可手动访问该网址，再用手机米家 App 扫描终端中的二维码并确认登录。若账号下有多台插座，扫码后会列出候选设备供你按编号选择；不会输出 token。若已有 `~/.config/xiaomi-power/config.json`，会直接复用。完成后，在 Plasma 小组件列表中添加 **Mi Power Monitor**。
