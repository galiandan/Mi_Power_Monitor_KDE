...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/mi-power-monitor-uninstall.XXXXXX")"
...
printf 'Downloading Mi Power Monitor KDE uninstall tools...\n'
...
if [[ -z "$source_dir" || ! -x "${source_dir}/uninstall.sh" ]]; then
    printf 'The downloaded archive does not contain uninstall.sh.\n' >&2
    exit 1
fi
